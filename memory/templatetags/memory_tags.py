...
def code_to_video(value):
    ...
    try:
        ...
        if match:
            for i in range(len(match)):
                strinfo = re.compile(r'\[\[('+match[i]+')\]\]')
                html = '<div class="videoDiv"><video class="html5video" controls="controls" width="550" height="380" poster="http://99.pyflask.sinaapp.com/_static/kinger/img/bg_video.png"><source src="' + match[i] + '" type="video/mp4" /></video></div>'
